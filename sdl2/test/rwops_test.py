...
def with_test_rw():
    ...
    rw = rwops.SDL_RWFromMem(buf, len(buf))
...
def test_SDL_RWFromFile(testfile_path):
    rw = rwops.SDL_RWFromFile(testfile_path, b"r")
    assert isinstance(rw.contents, rwops.SDL_RWops)
...
def test_SDL_RWFromMem():
    ...
    assert rwops.SDL_WriteLE16(rw, value) == 1
    assert buf.value == b"ab34"

def test_SDL_RWFromConstMem():
    buf = ctypes.create_string_buffer(b"1234")
    rw = rwops.SDL_RWFromConstMem(buf, len(buf))
    assert sdl2.SDL_GetError() == b""
    assert isinstance(rw.contents, rwops.SDL_RWops)
    ...
    assert rwops.SDL_WriteLE16(rw, value) == 0
    assert buf.value == b"1234"

def test_SDL_RWsize(with_test_rw):
    rw, buf = with_test_rw
    assert rwops.SDL_RWsize(rw) == len(buf)

def test_SDL_RWSeekTell(with_test_rw):
    rw, buf = with_test_rw
    seek_tests = [
        (rwops.RW_SEEK_END, 0, len(buf)), # Seek to end of RW
        (rwops.RW_SEEK_SET, 0, 0), # Seek to start of RW
        (rwops.RW_SEEK_CUR, 8, 8), # Seek 8 bytes forward from current pos
        (rwops.RW_SEEK_CUR, -3, 5), # Seek 3 bytes back from current pos
        (rwops.RW_SEEK_END, -4, len(buf) - 4), # Seek 4 bytes back from end
    ]
    for whence, offset, expected in seek_tests:
        pos1 = rwops.SDL_RWseek(rw, offset, whence)
        pos2 = rwops.SDL_RWtell(rw)
        assert pos1 == pos2
...
def test_SDL_RWread(with_test_rw):
    ...
    read = rwops.SDL_RWread(rw, readbuf, 1, 5)
    assert read == 5
...
def test_SDL_RWwrite(with_test_rw):
    ...
    rwops.SDL_RWseek(rw, -5, rwops.RW_SEEK_END) # NOTE: -5 here b/c of end byte
    written = rwops.SDL_RWwrite(rw, writebuf, 1, 4)
    assert written == 4
...
def test_SDL_RWclose():
    ...
    assert rwops.SDL_RWsize(rw) == len(buf)
    ret = rwops.SDL_RWclose(rw)
    assert sdl2.SDL_GetError() == b""
    assert ret == 0

def test_SDL_AllocFreeRW():
    rw = rwops.SDL_AllocRW()
    assert sdl2.SDL_GetError() == b""
    assert isinstance(rw.contents, rwops.SDL_RWops)
    rwops.SDL_FreeRW(rw)
    assert sdl2.SDL_GetError() == b""
...
def test_SDL_LoadFile_RW(testfile_path):
    ...
    data_p = rwops.SDL_LoadFile_RW(rw, ctypes.byref(datasize), 0)
    assert sdl2.SDL_GetError() == b""
...
def test_SDL_LoadFile(testfile_path):
    datasize = ctypes.c_size_t(0)
    data_p = rwops.SDL_LoadFile(testfile_path, ctypes.byref(datasize))
    assert sdl2.SDL_GetError() == b""
    assert datasize.value > 0
    data = ctypes.string_at(data_p, datasize.value)
    assert data[:19] == b"This is a test file"
...
def test_SDL_ReadU8(with_test_rw):
    rw, buf = with_test_rw
    assert chr(rwops.SDL_ReadU8(rw)) == "a"
    pos = rwops.SDL_RWseek(rw, 8, rwops.RW_SEEK_SET)
    assert pos == 8
    assert chr(rwops.SDL_ReadU8(rw)) == "i"
...
def test_SDL_WriteU8(with_test_rw):
    rw, buf = with_test_rw
    assert rwops.SDL_WriteU8(rw, ord("1")) == 1
...
def test_SDL_WriteBE16(with_test_rw):
    ...
    assert rwops.SDL_WriteBE16(rw, value) == 1
    assert buf.value == b"12cdefghijklmnop"
    rwops.SDL_RWseek(rw, 6, rwops.RW_SEEK_SET)
    assert rwops.SDL_WriteBE16(rw, value) == 1
...
def test_SDL_WriteLE32(with_test_rw):
    ...
    assert rwops.SDL_WriteLE32(rw, value) == 1
...
def test_SDL_WriteBE32(with_test_rw):
    ...
    assert rwops.SDL_WriteBE32(rw, value) == 1
...
def test_SDL_WriteLE64(with_test_rw):
    ...
    assert rwops.SDL_WriteLE64(rw, value) == 1
    assert buf.value == b"87654321ijklmnop"
    rwops.SDL_RWseek(rw, 8, rwops.RW_SEEK_SET)
    assert rwops.SDL_WriteLE64(rw, value) == 1
...
def test_SDL_WriteBE64(with_test_rw):
    ...
    assert rwops.SDL_WriteBE64(rw, value) == 1
...
class TestPythonRWops(object):
    ...
    def test_init(self, testfile_path):
        ...
        rw2 = rwops.rw_from_object(fileobj)
        assert isinstance(rw2, rwops.SDL_RWops)
        # NOTE: add to this? Not sure what else is useful to test.

    def test_size(self, test_buf):
        rw = rwops.rw_from_object(test_buf)
        assert rwops.SDL_RWsize(rw) == len(test_buf.getvalue())
        # Make sure querying size doesn't change the seek position
        assert rwops.SDL_RWtell(rw) == 0
        assert rwops.SDL_RWseek(rw, 8, rwops.RW_SEEK_SET)
        assert rwops.SDL_RWsize(rw) == len(test_buf.getvalue())
        assert rwops.SDL_RWtell(rw) == 8
        
    def test_seek(self, test_buf):
        rw = rwops.rw_from_object(test_buf)
        buflen = len(test_buf.getvalue())
        seek_tests = [
            (rwops.RW_SEEK_END, 0, buflen), # Seek to end of RW
            (rwops.RW_SEEK_SET, 0, 0), # Seek to start of RW
            (rwops.RW_SEEK_CUR, 8, 8), # Seek 8 bytes forward from current pos
            (rwops.RW_SEEK_CUR, -3, 5), # Seek 3 bytes back from current pos
            (rwops.RW_SEEK_END, -4, buflen - 4), # Seek 4 bytes back from end
        ]
        for whence, offset, expected in seek_tests:
            pos1 = rwops.SDL_RWseek(rw, offset, whence)
            pos2 = rwops.SDL_RWtell(rw)
    ...
    def test_write(self, test_buf):
        ...
        # Overwrite last 4 characters
        writebuf = ctypes.create_string_buffer(b"TEST")
        rwops.SDL_RWseek(rw, -4, rwops.RW_SEEK_END)
        written = rwops.SDL_RWwrite(rw, writebuf, 1, 4)
        assert written == 4
